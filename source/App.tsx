...
function App() {
    ...
    const ck = () => {
      ...
      matchs.forEach((m: any) => {
        ...
        if (dm > 55 && dm <= 65 && !notifiedMatchs.includes(k2)) {
          sendNotification('🎮 Match dans 1h !', 'DYNO vs ' + m.adversaire + ' • Préparez-vous !', 'm1h')
        ...
        if (dm > 10 && dm <= 20 && !notifiedMatchs.includes(k3)) {
          sendNotification('🔥 Match dans 15min !', 'DYNO vs ' + m.adversaire + ' • GO GO GO !', 'm15')
        ...
        if (dm >= -2 && dm <= 3 && !notifiedMatchs.includes(k4)) {
          sendNotification('⚡ C\'EST PARTI !', 'DYNO vs ' + m.adversaire + ' • EN COURS !', 'mnow')
  ...
  const toggleDispo = async (mid: string) => { if (!user) return; const m = matchs.find((x: any) => x.id === mid); if (!m) return; const d = m.disponibles || []; const i = m.indisponibles || []; const isDispo = d.includes(pseudo); if (!isDispo && myPass) { const tt = getMatchTimeType(m); const uk = tt === 'hp' ? 'hpUsed' : 'hcUsed'; const tk = tt === 'hp' ? 'hpTotal' : 'hcTotal'; const rem = (myPass[tk] || 0) - (myPass[uk] || 0); if (rem < 2) alert('⚠️ Plus que ' + rem + ' jeton(s) ' + (tt === 'hp' ? 'HP' : 'HC') + ' !'); const np = { ...myPass, [uk]: (myPass[uk] || 0) + 2 }; setMyPass(np); await updateDoc(doc(db, 'users', user.uid), { evaPass: np }) } else if (isDispo && myPass) { const tt = getMatchTimeType(m); const uk = tt === 'hp' ? 'hpUsed' : 'hcUsed'; const np = { ...myPass, [uk]: Math.max(0, (myPass[uk] || 0) - 2) }; setMyPass(np); await updateDoc(doc(db, 'users', user.uid), { evaPass: np }) }; addLog(isDispo ? 'Retrait dispo: ' + m.adversaire : 'Dispo: ' + m.adversaire); await updateDoc(doc(db, 'matchs', mid), { disponibles: isDispo ? d.filter((p: string) => p !== pseudo) : [...d, pseudo], indisponibles: i.filter((p: string) => p !== pseudo) }) }
              ...
              <div className="card-glow bg-black/30 rounded-3xl p-6 border border-[#D4AF37]/15 mb-6 relative overflow-hidden tab-content">
                ...
                <div className="flex items-center gap-5 mb-6">
                  <div className="relative group">
                    ...
                    <label className="absolute -bottom-2 -right-2 bg-[#D4AF37] text-black w-8 h-8 rounded-full flex items-center justify-center shadow-lg border-4 border-[#030303] cursor-pointer z-20 hover:scale-110 active:scale-90 transition-all">
                ...
                <div className="space-y-3 pt-4 border-t border-white/5">
                  <p className="text-[9px] text-gray-500 uppercase font-black tracking-widest">Ou coller une URL d'image</p>
                ...
                {myPass ? (
                  <div>
                    ...
                    <div className="bg-white/5 rounded-xl p-3 border border-white/5 mb-2">
                      ...
                      <button onClick={async () => { await updateDoc(doc(db, 'users', user.uid), { evaPass: myPass }); addLog('Pass modifié'); alert('✅!') }} className="w-full py-2 rounded-lg font-bold bg-gradient-to-r from-[#D4AF37] to-[#FFD700] text-black text-xs">💾 Sauvegarder</button>
                    </div>
            ...
            {!isAdmin ? (
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="card-glow bg-purple-500/10 rounded-2xl p-4 border border-purple-500/15"><p className="text-[8px] text-gray-500 uppercase font-black mb-1">Win Rate</p><p className="text-purple-400 font-bold text-xs">{winRate}%</p></div>
                </div>
                ...
                <div className="card-glow bg-black/30 rounded-3xl p-5 border border-[#D4AF37]/15">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <select value={nouveauMatch.arene} onChange={e => setNouveauMatch({ ...nouveauMatch, arene: e.target.value })} className="bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm">
                      <option value="Arène 1">Arène 1</option><option value="Arène 2">Arène 2</option>
                    </select>
                    <select value={nouveauMatch.type} onChange={e => setNouveauMatch({ ...nouveauMatch, type: e.target.value })} className="bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm">
                ...
                <div className="card-glow bg-black/30 rounded-3xl p-5 border border-[#D4AF37]/15">
                  <h3 className="text-xs font-bold text-[#D4AF37] mb-3 uppercase">🎬 Ajouter un Replay</h3>
                ...
                <div className="card-glow bg-black/30 rounded-3xl p-5 border border-[#D4AF37]/15">
                  <h3 className="text-xs font-bold text-[#D4AF37] mb-3 uppercase">✏️ Saisir les Scores</h3>
