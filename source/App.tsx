...
const DARK={bg:'radial-gradient(ellipse at top center,#100c00 0%,#050400 45%,#020200 100%)',card:'rgba(20,16,4,0.97)',cardBorder:'rgba(212,175,55,0.15)',text:'#f5f0e0',textMuted:'rgba(245,240,224,0.45)',input:'rgba(255,255,255,0.07)',inputBorder:'rgba(212,175,55,0.2)',navBg:'rgba(5,4,0,0.98)',navBorder:'rgba(212,175,55,0.1)',header:'rgba(6,5,0,0.97)'}
const LIGHT={bg:'radial-gradient(ellipse at top center,#fffbea 0%,#fdf6d3 45%,#faf0c0 100%)',card:'rgba(255,250,220,0.97)',cardBorder:'rgba(180,140,20,0.25)',text:'#1a1200',textMuted:'rgba(30,20,0,0.5)',input:'rgba(180,140,20,0.1)',inputBorder:'rgba(180,140,20,0.3)',navBg:'rgba(255,248,200,0.99)',navBorder:'rgba(180,140,20,0.25)',header:'rgba(255,248,200,0.99)'}
...
const CS={background:T.card,border:`1px solid ${T.cardBorder}`,boxShadow:isDark?'0 8px 40px rgba(0,0,0,0.8),inset 0 1px 0 rgba(212,175,55,0.08)':'0 4px 24px rgba(180,140,20,0.12)',backdropFilter:'blur(24px)'}
...
useEffect(()=>{const nextMatch=matchs.filter((m:any)=>!m.termine)[0];if(!nextMatch)return;const manifest={name:'DYNO Esport',short_name:'DYNO',start_url:'/',display:'standalone',theme_color:'#D4AF37',icons:[{src:LG,sizes:'192x192',type:'image/png'}],shortcuts:[{name:`DYNO vs ${nextMatch.adversaire}`,short_name:'Prochain',description:`${nextMatch.date} - ${nextMatch.horaire1}`,url:'/?tab=matchs',icons:[{src:LG,sizes:'96x96'}]}]};const blob=new Blob([JSON.stringify(manifest)],{type:'application/json'});const url=URL.createObjectURL(blob);const link=document.querySelector('link[rel="manifest"]')||document.createElement('link');(link as HTMLLinkElement).rel='manifest';(link as HTMLLinkElement).href=url;document.head.appendChild(link)},[matchs])
...
const updateScore=async()=>{if(!scoreEdit)return;if(scoreEdit.type==='Division'){const sm=scoreEdit.sousMatchs||[];const td=sm.reduce((a:number,s:any)=>a+parseInt(s.scoreDyno||0),0);const ta=sm.reduce((a:number,s:any)=>a+parseInt(s.scoreAdv||0),0);await updateDoc(doc(db,'matchs',scoreEdit.id),{sousMatchs:sm,scoreDyno:sm.length>0?td:parseInt(scoreEdit.scoreDyno||0),scoreAdversaire:sm.length>0?ta:parseInt(scoreEdit.scoreAdv||0),termine:true})}else{await updateDoc(doc(db,'matchs',scoreEdit.id),{scoreDyno:parseInt(scoreEdit.scoreDyno),scoreAdversaire:parseInt(scoreEdit.scoreAdv),termine:true})};setScoreEdit(null);alert('✅')}
...
const GBtn=({onClick,children,cls='',danger=false}:{onClick:()=>void,children:any,cls?:string,danger?:boolean})=>(
  <button onClick={onClick} className={`w-full py-4 rounded-2xl font-black text-sm tracking-wide transition-all active:scale-[0.97] select-none ${cls}`} style={danger?{background:'rgba(239,68,68,0.1)',color:'#f87171',border:'1px solid rgba(239,68,68,0.2)'}:{background:G2,color:'#000',boxShadow:'0 4px 24px rgba(212,175,55,0.3)'}}>{children}</button>
)
  ...
  <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{background:'rgba(0,0,0,0.93)',backdropFilter:'blur(28px)'}} onClick={onClose}>
    <div className="w-full sm:max-w-sm rounded-t-[2.5rem] sm:rounded-3xl max-h-[94vh] overflow-y-auto" style={{background:isDark?'linear-gradient(170deg,#161208,#0d0a04,#080500)':'linear-gradient(170deg,#fffdf0,#fff8d6)',border:`1px solid ${T.cardBorder}`,boxShadow:'0 -32px 80px rgba(0,0,0,0.9)'}} onClick={(e:any)=>e.stopPropagation()}>
      <div className="sticky top-0 pt-4 pb-3 px-6 z-10" style={{background:isDark?'rgba(14,11,3,0.98)':'rgba(255,252,224,0.98)',borderBottom:`1px solid ${T.cardBorder}`}}>
        <div className="w-10 h-1 rounded-full mx-auto mb-4" style={{background:'rgba(212,175,55,0.3)'}}/>
...
const ST=({icon,title}:{icon:string,title:string})=>(
  <div className="relative rounded-3xl p-7 mb-6 text-center overflow-hidden" style={{background:isDark?'linear-gradient(145deg,rgba(212,175,55,0.08),rgba(212,175,55,0.03),transparent)':'linear-gradient(145deg,rgba(212,175,55,0.18),rgba(212,175,55,0.08))',border:`1px solid ${T.cardBorder}`}}>
    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-px" style={{background:'linear-gradient(90deg,transparent,#D4AF37,transparent)'}}/>
    <img src={LG} alt="D" className="w-12 h-12 mx-auto mb-3"/>
    <h2 className="text-base font-black uppercase tracking-widest" style={{background:'linear-gradient(135deg,#D4AF37,#FFD700)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>{icon} {title}</h2>
  </div>
)
const SC=({value,label,color='#D4AF37',icon}:{value:any,label:string,color?:string,icon?:string})=>(
  <div className="rounded-2xl p-5 text-center relative overflow-hidden" style={{...CS}}>
    <div className="absolute inset-0 opacity-25" style={{background:`radial-gradient(ellipse at 50% 0%,${color}30,transparent 65%)`}}/>
    {icon&&<p className="text-2xl mb-1 relative">{icon}</p>}
    <p className="text-4xl font-black relative" style={{color,textShadow:`0 0 20px ${color}50`}}>{value}</p>
    <p className="text-[9px] uppercase tracking-widest mt-1.5 font-bold relative" style={{color:T.textMuted}}>{label}</p>
  </div>
)
const Tg=({children,color='gold'}:{children:any,color?:'gold'|'green'|'red'|'blue'|'gray'})=>{
  const c={gold:{bg:'rgba(212,175,55,0.12)',text:'#D4AF37',border:'rgba(212,175,55,0.25)'},green:{bg:'rgba(74,222,128,0.12)',text:'#4ade80',border:'rgba(74,222,128,0.25)'},red:{bg:'rgba(248,113,113,0.12)',text:'#f87171',border:'rgba(248,113,113,0.25)'},blue:{bg:'rgba(96,165,250,0.12)',text:'#60a5fa',border:'rgba(96,165,250,0.25)'},gray:{bg:'rgba(255,255,255,0.06)',text:T.textMuted,border:'rgba(255,255,255,0.1)'}}[color]
...
const navItems=[{t:'matchs',i:'📅',l:'Matchs'},{t:'historique',i:'🏆',l:'Résultats'},{t:'sondages',i:'🗳️',l:'Votes'},{t:'strats',i:'🎯',l:'Strats'},{t:'compos',i:'📋',l:'Compos'},{t:'fiches',i:'🔍',l:'Fiches'},{t:'notes',i:'📊',l:'Notes'},{t:'rec',i:'🎬',l:'Replays'},{t:'roster',i:'👥',l:'Roster'},{t:'stats',i:'📈',l:'Stats'},{t:'admin',i:'⚙️',l:'Admin'}]
...
.sg{background:linear-gradient(90deg,#B8860B,#D4AF37,#FFD700,#D4AF37,#B8860B);background-size:300% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:sh 4s linear infinite}
...
::-webkit-scrollbar{width:2px}::-webkit-scrollbar-thumb{background:rgba(212,175,55,.25);border-radius:10px}
`}</style>
...
<header className="sticky top-0 z-50" style={{background:T.header,backdropFilter:'blur(32px)',borderBottom:`1px solid ${T.navBorder}`,boxShadow:'0 4px 32px rgba(0,0,0,0.5)'}}>
  <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
    <div className="flex items-center gap-3">
      ...
      <div><h1 className="text-xl font-black leading-none sg">DYNO</h1><p className="text-[8px] tracking-[.3em] uppercase font-bold" style={{color:'rgba(212,175,55,.5)'}}>Esport</p></div>
    </div>
    <div className="flex items-center gap-2">
      <button onClick={toggleTheme} className="w-9 h-9 rounded-xl flex items-center justify-center active:scale-90" style={{background:isDark?'rgba(212,175,55,.1)':'rgba(180,140,20,.12)',border:`1px solid ${T.cardBorder}`}}><span className="text-base">{isDark?'☀️':'🌙'}</span></button>
      {user&&<button onClick={()=>setShowNotifSettings(true)} className="w-9 h-9 rounded-xl flex items-center justify-center active:scale-90" style={{background:notificationsEnabled?'rgba(212,175,55,.12)':'rgba(255,255,255,.05)',border:`1px solid ${T.cardBorder}`}}><span className="text-sm">{notificationsEnabled?'🔔':'🔕'}</span></button>}
      ...
      {user?(<button onClick={handleSignOut} className="flex items-center gap-2 px-3 py-2 rounded-xl active:scale-95" style={{background:'rgba(212,175,55,.08)',border:`1px solid ${T.cardBorder}`}}>
        ...
        <span className="text-xs font-bold max-w-[64px] truncate" style={{color:'#D4AF37'}}>{pseudo}</span>
  ...
  <Mo onClose={()=>setShowNotifSettings(false)} title="🔔 Notifications" sub="Choisis ce que tu veux recevoir">
    <div className="space-y-3 mb-5">
      {[{k:'match',label:'📅 Matchs à venir',desc:'1h, 15min et au lancement'},{k:'note',label:'📊 Nouvelles notes',desc:'Quand un joueur note'},{k:'commentaire',label:'💬 Commentaires',desc:"Quand quelqu'un commente"},{k:'strat',label:'🎯 Nouvelles strats',desc:'Quand une strat est ajoutée'},{k:'resultat',label:'🏆 Résultats',desc:'Quand un score est entré'}].map(({k,label,desc})=>(
        <div key={k} className="flex items-center justify-between p-4 rounded-2xl" style={{background:isDark?'rgba(255,255,255,.04)':'rgba(180,140,20,.07)',border:`1px solid ${T.cardBorder}`}}>
...
{pullDistance>0&&<div className="flex justify-center mb-3" style={{height:pullDistance}}><span className={`text-2xl ${pullDistance>60?'rotate-180':''}`} style={{color:'#D4AF37'}}>{isRefreshing?'⏳':'↓'}</span></div>}
...
<div className="ce"><ST icon="📅" title="Prochains Matchs"/>
  <div className="flex justify-end mb-4">
    <div className="flex rounded-2xl overflow-hidden p-1" style={{background:'rgba(255,255,255,.03)',border:`1px solid ${T.cardBorder}`}}>
      {(['list','grid'] as const).map(v=><button key={v} onClick={()=>setViewMode(v)} className="px-4 py-1.5 rounded-xl text-xs font-bold" style={viewMode===v?{background:'rgba(212,175,55,.18)',color:'#D4AF37'}:{color:T.textMuted}}>{v==='list'?'☰':'⊞'}</button>)}
    </div>
  </div>
  ...
  :<div className={viewMode==='grid'?'grid grid-cols-2 gap-3':'space-y-4'}>{prochainsMatchs.map((match:any,idx:number)=>(
    <div key={match.id} className="rounded-3xl overflow-hidden ce" style={{...CS,animationDelay:`${idx*.1}s`}}>
      <div className="h-px w-full" style={{background:'linear-gradient(90deg,transparent,rgba(212,175,55,.25),transparent)'}}/>
      <div className="px-5 pt-4 pb-3 flex items-center justify-between"><Bdg type={match.type}/><span className="text-xs font-bold" style={{color:'rgba(212,175,55,.7)'}}>{fdf(match.date)}</span></div>
      {countdowns[match.id]&&<div className="mx-4 mb-3 rounded-2xl px-4 py-3 text-center" style={countdowns[match.id]==='EN COURS'?{background:'rgba(239,68,68,.12)',border:'1px solid rgba(239,68,68,.2)'}:{background:'rgba(212,175,55,.06)',border:'1px solid rgba(212,175,55,.12)'}}>
        <p className="text-[8px] uppercase tracking-widest mb-1.5 font-bold" style={{color:T.textMuted}}>Countdown</p>
        {countdowns[match.id]==='EN COURS'?<div className="flex items-center justify-center gap-2"><div className="w-2 h-2 rounded-full ld" style={{background:'#f87171'}}/><p className="text-base font-black" style={{color:'#f87171'}}>EN COURS</p></div>:<p className="text-xl font-black font-mono" style={{background:'linear-gradient(90deg,#D4AF37,#FFD700)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>{countdowns[match.id]}</p>}
      </div>}
      <div className="px-5 pb-4 flex items-center gap-4">
        <img src={LG} alt="D" className="w-12 h-12 rounded-2xl flex-shrink-0"/>
        <div className="flex-1 min-w-0"><p className="text-[9px] font-black uppercase tracking-widest mb-0.5" style={{color:'rgba(212,175,55,.45)'}}>DYNO vs</p><p className="text-base font-black truncate" style={{color:T.text}}>{match.adversaire}</p><p className="text-[10px] mt-0.5" style={{color:T.textMuted}}>🏟️ {match.arene}</p></div>
      </div>
      <div className="mx-4 mb-3 rounded-xl px-4 py-2.5 flex items-center gap-2.5" style={{background:'rgba(212,175,55,.05)',border:'1px solid rgba(212,175,55,.1)'}}><span>⏰</span><span className="text-sm font-black" style={{color:'#D4AF37'}}>{match.horaires?.join(' / ')||match.horaire1||'20:00'}</span></div>
      {(match.disponibles||[]).length>0&&<div className="mx-4 mb-2 rounded-xl px-4 py-3" style={{background:'rgba(212,175,55,.05)',border:'1px solid rgba(212,175,55,.08)'}}><p className="text-[9px] font-black uppercase tracking-widest mb-2" style={{color:'rgba(212,175,55,.5)'}}>✅ Dispo ({(match.disponibles||[]).length})</p><div className="flex flex-wrap gap-1.5">{(match.disponibles||[]).map((p:string,i:number)=><Tg key={i} color="gold">{p}</Tg>)}</div></div>}
      {(match.indisponibles||[]).length>0&&<div className="mx-4 mb-3 rounded-xl px-4 py-3" style={{background:'rgba(239,68,68,.05)',border:'1px solid rgba(239,68,68,.08)'}}><p className="text-[9px] font-black uppercase tracking-widest mb-2" style={{color:'rgba(239,68,68,.5)'}}>🚫 Indispo ({(match.indisponibles||[]).length})</p><div className="flex flex-wrap gap-1.5">{(match.indisponibles||[]).map((p:string,i:number)=><Tg key={i} color="red">{p}</Tg>)}</div></div>}
...
<div className="ce"><ST icon="🏆" title="Résultats"/>
  <div className="grid grid-cols-2 gap-3 mb-6"><SC value={victoires} label="Victoires" color="#D4AF37" icon="🏆"/><SC value={defaites} label="Défaites" color="#f87171" icon="❌"/></div>
  ...
  :<div className="space-y-3">{historique.map((match:any,idx:number)=>{
    ...
    return<div key={match.id} className="rounded-3xl overflow-hidden ce" style={{...CS,animationDelay:`${idx*.05}s`}}>
      <div className="h-px w-full" style={{background:win?'linear-gradient(90deg,transparent,rgba(212,175,55,.35),transparent)':'linear-gradient(90deg,transparent,rgba(239,68,68,.25),transparent)'}}/>
      <div className="px-5 pt-4 pb-3 flex items-center justify-between">
        <span className="px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest" style={win?{background:'rgba(212,175,55,.15)',color:'#D4AF37',border:'1px solid rgba(212,175,55,.25)'}:{background:'rgba(239,68,68,.12)',color:'#f87171',border:'1px solid rgba(239,68,68,.22)'}}>{win?'🏆 Victoire':'❌ Défaite'}</span>
        <div className="flex items-center gap-2">
          {isAdmin&&<button onClick={()=>setScoreEdit({id:match.id,adversaire:match.adversaire,type:match.type,scoreDyno:String(match.scoreDyno||0),scoreAdv:String(match.scoreAdversaire||0),sousMatchs:match.sousMatchs?[...match.sousMatchs]:[],_newSubAdv:'',_newSubScoreDyno:'',_newSubScoreAdv:''})} className="px-2.5 py-1.5 rounded-xl text-[10px] font-bold active:scale-95" style={{background:'rgba(212,175,55,.1)',color:'rgba(212,175,55,.8)',border:'1px solid rgba(212,175,55,.18)'}}>✏️</button>}
          <span className="text-xs font-medium" style={{color:T.textMuted}}>{fdf(match.date)}</span>
        </div>
      </div>
      <div className="px-6 pb-5 flex items-center justify-between">
        <div className="text-center flex-1"><p className="text-[9px] font-black uppercase tracking-widest mb-2" style={{color:'rgba(212,175,55,.45)'}}>DYNO</p><p className="text-7xl font-black leading-none" style={{background:'linear-gradient(135deg,#D4AF37,#FFD700)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>{match.scoreDyno}</p></div>
      ...
      {match.sousMatchs?.length>0&&<div className="mx-4 mb-4 rounded-2xl overflow-hidden" style={{border:'1px solid rgba(245,158,11,.15)'}}>
        ...
        {match.sousMatchs.map((sm:any,i:number)=><div key={i} className="px-4 py-2.5 flex items-center justify-between" style={{borderTop:i>0?`1px solid ${T.cardBorder}`:'none'}}>
          ...
          <span className="text-xs font-black"><span style={{color:'#D4AF37'}}>{sm.scoreDyno}</span><span style={{color:T.textMuted}}> — </span><span style={{color:T.textMuted}}>{sm.scoreAdv}</span></span>
        </div>)}
      </div>}
...
<div className="ce"><ST icon="🗳️" title="Sondages"/>
  ...
  :<div className="space-y-4">{sondages.map((s:any,idx:number)=>{
    ...
    return<div key={s.id} className="rounded-3xl overflow-hidden ce" style={{...CS,animationDelay:`${idx*.05}s`}}>
      <div className="h-px w-full" style={{background:'linear-gradient(90deg,transparent,rgba(212,175,55,.2),transparent)'}}/>
      ...
      <div className="p-4 space-y-2.5">
        {Object.keys(opts).map(k=>{const o=opts[k],votes=o.votes?.length||0,pct=totalVotes>0?Math.round((votes/totalVotes)*100):0,isMyVote=userVote===k
          return<button key={k} onClick={()=>!isFinished&&user&&voterSondage(s.id,k)} disabled={isFinished||!user} className="w-full text-left rounded-2xl overflow-hidden transition-all active:scale-[.98]" style={{border:`1px solid ${isMyVote?'rgba(212,175,55,.35)':T.cardBorder}`}}>
            <div className="px-4 py-3 relative">
              <div className="absolute inset-0 rounded-2xl transition-all duration-500" style={{background:isMyVote?'rgba(212,175,55,.15)':'rgba(255,255,255,.02)',width:`${userVote?pct:0}%`}}/>
              <div className="relative flex items-center justify-between">
                <div className="flex items-center gap-2.5">{isMyVote&&<span className="text-xs">✅</span>}<span className="text-sm font-bold" style={{color:isMyVote?'#D4AF37':T.text}}>{o.label}</span></div>
                <div className="flex items-center gap-2">{userVote&&<span className="text-xs font-black" style={{color:isMyVote?'#D4AF37':T.textMuted}}>{pct}%</span>}<span className="text-[10px]" style={{color:T.textMuted}}>{votes}</span></div>
              </div>
            </div>
          </button>
        })}
  ...
  {showAddSondage&&<Mo onClose={()=>setShowAddSondage(false)} title="🗳️ Nouveau Sondage" sub="Vote pour l'équipe">
    <div className="space-y-3 mb-5">
      ...
      <p className="text-[10px] font-black uppercase tracking-widest" style={{color:'rgba(212,175,55,.6)'}}>Options</p>
      ...
      {nouveauSondage.options.length<6&&<button onClick={()=>setNouveauSondage({...nouveauSondage,options:[...nouveauSondage.options,'']})} className="w-full py-2.5 rounded-xl text-xs font-bold" style={{background:'rgba(212,175,55,.08)',color:'#D4AF37',border:'1px solid rgba(212,175,55,.18)'}}>➕ Option</button>}
    </div>
...
<div className="ce"><ST icon="🎯" title="Stratégies"/>
  {user&&<GBtn onClick={()=>setShowAddStrat(true)} cls="mb-5">➕ Nouvelle Stratégie</GBtn>}
...
<div className="ce"><ST icon="📋" title="Compositions"/>
  ...
  :<div className="space-y-3">{compos.map((c:any,idx:number)=><div key={c.id} className="rounded-3xl p-5 ce" style={{...CS,animationDelay:`${idx*.05}s`}}>
    ...
    <div className="flex flex-wrap gap-1.5">{c.joueurs?.map((j:string,i:number)=><Tg key={i} color="gold">{j}</Tg>)}</div>
  </div>)}</div>}
  {showAddCompo&&<Mo onClose={()=>{setShowAddCompo(false);setSelectedMapCompo('');setCompoJoueurs([])}} title="📋 Nouvelle Compo" sub="Map et joueurs">
    <div className="space-y-4 mb-5">
      <div><p className="text-[9px] font-black uppercase tracking-widest mb-2.5" style={{color:'rgba(212,175,55,.7)'}}>🗺️ Map</p><div className="grid grid-cols-3 gap-1.5">{AM.map(m=><button key={m} onClick={()=>setSelectedMapCompo(m)} className="px-2 py-2.5 rounded-xl text-[10px] font-bold active:scale-95" style={selectedMapCompo===m?{background:'rgba(212,175,55,.18)',color:'#D4AF37',border:'1px solid rgba(212,175,55,.3)'}:{background:T.input,color:T.textMuted,border:`1px solid ${T.inputBorder}`}}>{m}</button>)}</div></div>
      <div><p className="text-[9px] font-black uppercase tracking-widest mb-2.5" style={{color:'rgba(212,175,55,.7)'}}>👥 Joueurs</p><div className="grid grid-cols-2 gap-1.5">{joueurs.filter((j:any)=>j.actif!==false).map((j:any)=><button key={j.id} onClick={()=>toggleCompoJoueur(j.pseudo)} className="px-3 py-2.5 rounded-xl text-xs font-bold active:scale-95" style={compoJoueurs.includes(j.pseudo)?{background:'rgba(74,222,128,.18)',color:'#4ade80',border:'1px solid rgba(74,222,128,.3)'}:{background:T.input,color:T.textMuted,border:`1px solid ${T.inputBorder}`}}>{j.pseudo}</button>)}</div></div>
    </div>
...
<div className="ce"><ST icon="🔍" title="Fiches Adversaires"/>
  {user&&<GBtn onClick={()=>setShowAddFiche(true)} cls="mb-5">➕ Nouvelle Fiche</GBtn>}
    ...
    return<div key={match.id} className="rounded-3xl overflow-hidden ce" style={{...CS,animationDelay:`${idx*.05}s`}}>
      <div className="px-5 pt-4 pb-3" style={{borderBottom:`1px solid ${T.cardBorder}`}}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="px-2 py-1 rounded-full text-[9px] font-black" style={win?{background:'rgba(212,175,55,.15)',color:'#D4AF37'}:{background:'rgba(239,68,68,.12)',color:'#f87171'}}>{match.scoreDyno}–{match.scoreAdversaire}</span>
...
<div className="ce"><ST icon="👥" title="Roster"/>
  ...
  <div className="space-y-2.5">{joueurs.filter((j:any)=>j.actif!==false).map((j:any,idx:number)=><div key={j.id} className="rounded-2xl p-4 flex items-center gap-4 ce" style={{...CS,animationDelay:`${idx*.05}s`}}>
    <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl flex-shrink-0" style={{background:'linear-gradient(135deg,rgba(212,175,55,.2),rgba(212,175,55,.06))',border:'1px solid rgba(212,175,55,.2)',color:'#D4AF37'}}>{j.pseudo[0]?.toUpperCase()}</div>
    <div className="flex-1 min-w-0"><p className="font-black" style={{color:T.text}}>{j.pseudo}</p><p className="text-[10px] mt-0.5" style={{color:T.textMuted}}>🎮 {j.role}</p></div>
...
<div className="ce"><ST icon="📈" title="Statistiques"/>
  <div className="grid grid-cols-3 gap-3 mb-5"><SC value={`${winRate}%`} label="Win Rate" color="#D4AF37"/><SC value={victoires} label="Victoires" color="#4ade80"/><SC value={defaites} label="Défaites" color="#f87171"/></div>
  {winRateData.length>1&&<div className="rounded-3xl p-5 mb-4" style={CS}>
    <p className="text-xs font-black uppercase tracking-widest mb-4" style={{color:'rgba(212,175,55,.7)'}}>📈 Évolution Win Rate</p>
    <ResponsiveContainer width="100%" height={160}><LineChart data={winRateData}><XAxis dataKey="name" tick={{fontSize:9,fill:T.textMuted}} axisLine={false} tickLine={false}/><YAxis domain={[0,100]} tick={{fontSize:9,fill:T.textMuted}} axisLine={false} tickLine={false} tickFormatter={(v:any)=>`${v}%`}/><Tooltip contentStyle={{background:isDark?'#111008':'#fffdf0',border:`1px solid ${T.cardBorder}`,borderRadius:12,fontSize:11}} formatter={(v:any)=>[`${v}%`,'Win Rate']}/><Line type="monotone" dataKey="wr" stroke="#D4AF37" strokeWidth={2.5} dot={{fill:'#D4AF37',r:3}} activeDot={{r:5,fill:'#FFD700'}}/></LineChart></ResponsiveContainer>
  </div>}
  {typeData.length>0&&<div className="rounded-3xl p-5 mb-4" style={CS}>
    <p className="text-xs font-black uppercase tracking-widest mb-4" style={{color:'rgba(212,175,55,.7)'}}>📊 Par type</p>
    <ResponsiveContainer width="100%" height={140}><BarChart data={typeData} barCategoryGap="30%"><XAxis dataKey="name" tick={{fontSize:9,fill:T.textMuted}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:9,fill:T.textMuted}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{background:isDark?'#111008':'#fffdf0',border:`1px solid ${T.cardBorder}`,borderRadius:12,fontSize:11}}/><Bar dataKey="matchs" fill="rgba(212,175,55,.2)" radius={[6,6,0,0]} name="Total"/><Bar dataKey="victoires" radius={[6,6,0,0]} name="Victoires">{typeData.map((_:any,i:number)=><Cell key={i} fill="#D4AF37"/>)}</Bar></BarChart></ResponsiveContainer>
  </div>}
  {notesChartData.length>0&&<div className="rounded-3xl p-5 mb-4" style={CS}>
    <p className="text-xs font-black uppercase tracking-widest mb-4" style={{color:'rgba(212,175,55,.7)'}}>🧠 Notes par joueur</p>
  ...
  <div className="rounded-3xl p-6 mb-5" style={CS}>
    <p className="text-xs font-black uppercase tracking-widest mb-5" style={{color:'rgba(212,175,55,.7)'}}>📊 Répartition</p>
    <div className="space-y-4">{[{label:'🏆 Victoires',value:victoires,total:totalMatchs,color:'#D4AF37'},{label:'❌ Défaites',value:defaites,total:totalMatchs,color:'#f87171'}].map(({label,value,total,color})=>
      <div key={label}>
  ...
  {showBilan&&(()=>{const b=genBilan();return(
    <Mo onClose={()=>setShowBilan(false)} title={`📊 Bilan ${b.nom}`} sub="Performances du mois">
      <div className="space-y-4 mb-5">
        ...
        <div className="rounded-2xl p-6 text-center" style={{background:'rgba(212,175,55,.08)',border:'1px solid rgba(212,175,55,.15)'}}><p className="text-[9px] font-black uppercase tracking-widest mb-2" style={{color:'rgba(212,175,55,.5)'}}>Win Rate</p><p className="text-6xl font-black sg">{b.wr}%</p></div>
  ...
  {!isAdmin?(
    ...
    <div className="space-y-4">
      <div className="rounded-3xl p-5" style={CS}>
        <p className="text-xs font-black uppercase tracking-widest mb-4" style={{color:'rgba(212,175,55,.8)'}}>➕ Nouveau Match</p>
          ...
          {nouveauMatch.type==='Division'&&<div className="rounded-2xl p-4" style={{background:'rgba(245,158,11,.07)',border:'1px solid rgba(245,158,11,.18)'}}>
            ...
            {nouveauMatch.sousMatchs.length>0&&<div className="space-y-1.5 mb-3">{nouveauMatch.sousMatchs.map((sm:any,i:number)=>
              <div key={i} className="flex items-center justify-between px-3 py-2.5 rounded-xl" style={{background:isDark?'rgba(255,255,255,.06)':'rgba(0,0,0,.06)'}}>
                ...
                <div className="flex items-center gap-2"><span className="text-xs font-black" style={{color:'#D4AF37'}}>{sm.scoreDyno}</span><span style={{color:T.textMuted}}>—</span><span className="text-xs font-black" style={{color:T.textMuted}}>{sm.scoreAdv}</span><button onClick={()=>supprimerSousMatch(i)} className="ml-1 font-black text-red-400">×</button></div>
              </div>
            )}</div>}
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input type="number" placeholder="Score DYNO" value={newSubScoreDyno} onChange={(e:any)=>setNewSubScoreDyno(e.target.value)} className={`${iCls} text-center font-black text-sm py-2.5`} style={{background:'rgba(212,175,55,.1)',border:'1px solid rgba(212,175,55,.25)',color:'#D4AF37'}}/>
      ...
      <div className="rounded-3xl p-5" style={CS}>
        <p className="text-xs font-black uppercase tracking-widest mb-4" style={{color:'rgba(212,175,55,.8)'}}>🎬 Ajouter un Replay</p>
        <div className="space-y-2.5">
      ...
      <div className="rounded-3xl p-5" style={CS}>
        <p className="text-xs font-black uppercase tracking-widest mb-4" style={{color:'rgba(212,175,55,.8)'}}>✏️ Modifier les Scores</p>
      ...
      <div className="space-y-3 mb-5">
        ...
        {(scoreEdit.sousMatchs||[]).length>0&&<div className="space-y-1.5">{(scoreEdit.sousMatchs||[]).map((sm:any,i:number)=>
          <div key={i} className="flex items-center justify-between px-4 py-3 rounded-2xl" style={{background:isDark?'rgba(255,255,255,.05)':'rgba(0,0,0,.05)',border:`1px solid ${T.cardBorder}`}}>
            ...
            <div className="flex items-center gap-2"><span className="text-sm font-black" style={{color:'#D4AF37'}}>{sm.scoreDyno}</span><span style={{color:T.textMuted}}>—</span><span className="text-sm font-black" style={{color:T.textMuted}}>{sm.scoreAdv}</span><button onClick={()=>supprimerSousMatchEdit(i)} className="ml-2 font-black text-red-400">×</button></div>
          </div>
        )}</div>}
        <div className="rounded-2xl p-4 space-y-2" style={{background:'rgba(245,158,11,.07)',border:'1px solid rgba(245,158,11,.18)'}}>
          ...
          <div className="grid grid-cols-2 gap-2">
            <input type="number" placeholder="DYNO" value={scoreEdit._newSubScoreDyno||''} onChange={(e:any)=>setScoreEdit({...scoreEdit,_newSubScoreDyno:e.target.value})} className={`${iCls} text-center font-black py-2.5`} style={{background:'rgba(212,175,55,.1)',border:'1px solid rgba(212,175,55,.25)',color:'#D4AF37'}}/>
        ...
        {(scoreEdit.sousMatchs||[]).length>0&&<div className="rounded-xl p-4 text-center" style={{background:'rgba(212,175,55,.08)',border:'1px solid rgba(212,175,55,.15)'}}>
          <p className="text-[9px] uppercase tracking-widest mb-1.5" style={{color:'rgba(212,175,55,.5)'}}>Total</p>
          <p className="text-3xl font-black"><span style={{color:'#D4AF37'}}>{(scoreEdit.sousMatchs||[]).reduce((a:number,s:any)=>a+parseInt(s.scoreDyno||0),0)}</span><span style={{color:T.textMuted}}> — </span><span style={{color:T.textMuted}}>{(scoreEdit.sousMatchs||[]).reduce((a:number,s:any)=>a+parseInt(s.scoreAdv||0),0)}</span></p>
        </div>}
      </div>
    ):(
      <div className="grid grid-cols-2 gap-4 mb-5">
        <div><p className="text-[10px] font-bold uppercase mb-2" style={{color:'rgba(212,175,55,.6)'}}>DYNO</p><input type="number" placeholder="0" value={scoreEdit.scoreDyno} onChange={(e:any)=>setScoreEdit({...scoreEdit,scoreDyno:e.target.value})} className="w-full rounded-2xl px-4 py-6 text-center text-5xl font-black focus:outline-none" style={{background:'rgba(212,175,55,.1)',border:'1px solid rgba(212,175,55,.22)',color:'#D4AF37'}}/></div>
...
<nav className="fixed bottom-0 left-0 right-0 z-50" style={{background:T.navBg,backdropFilter:'blur(32px)',borderTop:`1px solid ${T.navBorder}`,boxShadow:'0 -8px 40px rgba(0,0,0,0.6)'}}>
  <div className="max-w-lg mx-auto flex" style={{overflowX:'auto',scrollbarWidth:'none' as any,WebkitOverflowScrolling:'touch'} as any}>
    {navItems.map(({t,i,l})=>(
      <button key={t} onClick={()=>setActiveTab(t)} className="flex-shrink-0 flex flex-col items-center justify-center py-2.5 relative transition-all active:scale-90" style={{color:activeTab===t?'#D4AF37':T.textMuted,minWidth:`${100/navItems.length}%`}}>
        {activeTab===t&&<div className="absolute top-0 left-1/2 -translate-x-1/2" style={{width:'28px',height:'2.5px',background:'linear-gradient(90deg,transparent,#D4AF37,transparent)',boxShadow:'0 0 10px rgba(212,175,55,.6)'}}/>}
        <span className="text-lg leading-none mb-0.5">{i}</span>
        <span className="text-[7px] font-black uppercase tracking-wide leading-none whitespace-nowrap" style={{color:activeTab===t?'rgba(212,175,55,.8)':T.textMuted}}>{l}</span>
      </button>
    ))}
  </div>
</nav>
...
{!user&&(
  <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{background:'rgba(0,0,0,0.97)',backdropFilter:'blur(32px)'}}>
    <div className="w-full sm:max-w-sm rounded-t-[2.5rem] sm:rounded-3xl" style={{background:isDark?'linear-gradient(170deg,#181208,#0e0a04,#080500)':'linear-gradient(170deg,#fffdf0,#fff8d6)',border:`1px solid ${T.cardBorder}`}}>
      <div className="w-10 h-1 rounded-full mx-auto mt-4 sm:hidden" style={{background:'rgba(212,175,55,.25)'}}/>
      <div className="p-8">
        ...
        <div className="text-center pt-4" style={{borderTop:`1px solid ${T.cardBorder}`}}>
          {isSignUp?<button onClick={()=>setIsSignUp(false)} className="text-xs" style={{color:T.textMuted}}>Déjà un compte ? <span className="font-black" style={{color:'#D4AF37'}}>Se connecter</span></button>:<button onClick={()=>setIsSignUp(true)} className="text-xs" style={{color:T.textMuted}}>Pas de compte ? <span className="font-black" style={{color:'#D4AF37'}}>S'inscrire</span></button>}
        </div>
      </div>
    </div>
  </div>
