...
const fef=(m:any)=>{if(!m)return'';if(m.scoreDyno===undefined||m.scoreAdversaire===undefined)return'À venir';return`DYNO ${m.scoreDyno} - ${m.scoreAdversaire} ${m.adversaire}`}
...
{user?<><span>{pseudo}</span><button onClick={handleSignOut}>Déconnexion</button></>:<><button onClick={()=>setIsSignUp(!isSignUp)}>{isSignUp?'Connexion':'Inscription'}</button></>}
...
<option>Arène 1</option><option>Arène 2</option><option>Arène 3</option>
</select>
...
{replays.map(r=><div key={r.id} className="replay-card"><a href={r.lien} target="_blank">{r.titre}</a></div>)}
</div>}
...
<select value={selectedMapCompo} onChange={e=>setSelectedMapCompo(e.target.value)}><option value="">Sélectionner une map</option>{AM.map(map=><option key={map} value={map}>{map}</option>)}</select>
...
{compos.map(c=><div key={c.id} className="compo-card"><div>🗺️ {c.map}</div><div>Joueurs: {c.joueurs.join(', ')}</div></div>)}
</div>}
...
<textarea placeholder="Ce qui a bien fonctionné..." value={nouvelleAnalyse.bien} onChange={e=>setNouvelleAnalyse({...nouvelleAnalyse,bien:e.target.value})}/>
<textarea placeholder="Ce qui a mal fonctionné..." value={nouvelleAnalyse.mal} onChange={e=>setNouvelleAnalyse({...nouvelleAnalyse,mal:e.target.value})}/>
...
<input placeholder="Nom de l'équipe adverse" value={nouvelleFiche.adversaire} onChange={e=>setNouvelleFiche({...nouvelleFiche,adversaire:e.target.value})}/>
...
<textarea placeholder="Notes supplémentaires..." value={nouvelleFiche.notes} onChange={e=>setNouvelleFiche({...nouvelleFiche,notes:e.target.value})}/>
<button onClick={ajouterFiche}>Enregistrer</button>
</div>}
{fichesAdversaires.map(f=><div key={f.id} className="fiche-card"><div>⚔️ {f.adversaire}</div><div><strong>Forces:</strong> {f.forces}</div><div><strong>Faiblesses:</strong> {f.faiblesses}</div>{f.notes&&<div><strong>Notes:</strong> {f.notes}</div>}</div>)}
</div>}

{!user&&<div className="auth-modal">
{isSignUp?<div className="auth-form"><h3>Inscription</h3><input placeholder="Pseudo" value={pseudo} onChange={e=>setPseudo(e.target.value)}/><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)}/><input type="password" placeholder="Mot de passe" value={authPassword} onChange={e=>setAuthPassword(e.target.value)}/><button onClick={handleSignUp}>S'inscrire</button><button onClick={()=>setIsSignUp(false)}>Déjà un compte?</button></div>:<div className="auth-form"><h3>Connexion</h3><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)}/><input type="password" placeholder="Mot de passe" value={authPassword} onChange={e=>setAuthPassword(e.target.value)}/><button onClick={handleSignIn}>Se connecter</button><button onClick={()=>setIsSignUp(true)}>Créer un compte</button></div>}
</div>}
...
<div><a href={YT} target="_blank">YouTube</a></div>
</footer>
</div>
)
}
